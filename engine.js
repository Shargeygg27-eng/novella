...
tg.expand(); // Разворачиваем на максимально доступную высоту
if (tg.requestFullscreen) {
    tg.requestFullscreen(); // Если версия ТГ новая - включаем честный Fullscreen
}
...
function startGame() {
    ...
    // Запускаем предзагрузку для первой сцены (Начало игры)
...
function generateChaptersGrid() {
    ...
    // Точки входа для первых 4-х глав
...
// === 3. КИНЕМАТОГРАФИЧНАЯ ЗАГРУЗКА СЦЕНЫ ===
function loadScene(sceneId, isInitialLoad = false) {
    // Если сюжет закончен и нужно вернуться в меню (например из концовки 4 главы)
